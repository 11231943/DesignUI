...
public abstract class TabLayout extends HorizontalScrollView {
    ...
    private int mTabMaxWidth = Short.MAX_VALUE;
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // Add the TabStrip
        mTabStrip = createTabStrip();
        addView(mTabStrip, LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);

        TypedArray tarr = context.obtainStyledAttributes(attrs, R.styleable.TabLayout);
        mTabStrip.setSelectedIndicatorMargin(tarr.getDimensionPixelSize(
                R.styleable.TabLayout_tabIndicatorMargin, 0));
        mTabStrip.setSelectedIndicatorRadius(tarr.getDimensionPixelSize(
                R.styleable.TabLayout_tabIndicatorRadius, 0));
        tarr.recycle();
        ...
        if (a.hasValue(android.support.design.R.styleable.TabLayout_tabSelectedTextColor)) {
            ...
            final int selected = a.getColor(
                    android.support.design.R.styleable.TabLayout_tabSelectedTextColor, 0);
        ...
        mTabBackgroundResId = a.getResourceId(
                android.support.design.R.styleable.TabLayout_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(
                android.support.design.R.styleable.TabLayout_tabContentStart, 0);
        mMode = a.getInt(android.support.design.R.styleable.TabLayout_tabMode, MODE_FIXED);
        mTabGravity = a.getInt(android.support.design.R.styleable.TabLayout_tabGravity,
                GRAVITY_FILL);
        a.recycle();

        // TODO add attr for these
        final Resources res = getResources();
        mTabTextMultiLineSize = res.getDimensionPixelSize(
                android.support.design.R.dimen.design_tab_text_size_2line);
     ...
     */
    public void setScrollPosition(int oldPosition, int position, float positionOffset, boolean updateSelectedText) {
        ...
        scrollTo(calculateScrollXForTab(oldPosition, position, positionOffset), 0);
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        ss.selectedPosition = getSelectedTabPosition();
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        if (ss.selectedPosition > 0 && ss.selectedPosition < mTabs.size()) {
            selectTab(mTabs.get(ss.selectedPosition));
        }
    }
    ...
    private void animateToTab(int oldPosition, int newPosition) {
        ...
        if (getWindowToken() == null || !ViewCompat.isLaidOut(this)
            ...
            // position now
            setScrollPosition(oldPosition, newPosition, 0f, true);
            return;
        }

        final int startScrollX = getScrollX();
        final int targetScrollX = calculateScrollXForTab(oldPosition, newPosition, 0);
    ...
    public void selectTab(Tab tab, boolean updateIndicator) {
        int oldPos = mSelectedTab == null ? Tab.INVALID_POSITION : mSelectedTab.getPosition();
        if (mSelectedTab == tab) {
            if (mSelectedTab != null) {
                if (mOnTabSelectedListener != null) {
                    mOnTabSelectedListener.onTabReselected(mSelectedTab);
                }
                animateToTab(oldPos, tab.getPosition());
            }
        } else {
            if (updateIndicator) {
                ...
                if ((mSelectedTab == null || mSelectedTab.getPosition() == Tab.INVALID_POSITION)
                    ...
                    setScrollPosition(oldPos, newPosition, 0f, true);
                } else {
                    animateToTab(oldPos, newPosition);
                }
            }
    ...
    private int calculateScrollXForTab(int oldPosition, int position, float positionOffset) {
        if (mMode == MODE_SCROLLABLE) {
            ...
            int scrollX = selectedChild.getLeft()
                    + ((int) ((selectedWidth + nextWidth) * positionOffset * 0.5f))
                    + (selectedChild.getWidth() / 2)
                    - (getWidth() / 2);
            if (oldPosition != Tab.INVALID_POSITION) {
                final View oldChild = mTabStrip.getChildAt(oldPosition);
                final int oldWidth = oldChild != null ? oldChild.getWidth() / 2 : 0;
                if (oldPosition < position && scrollX <= 0) {
                    scrollX = getScrollX() + oldWidth;
                } else if (oldPosition > position && scrollX >= getWidth()) {
                    scrollX = getScrollX() - oldWidth;
                }
            }
            ...
            if (mCustomView == null) {
                ...
                if (mIconView == null) {
                    ImageView iconView = (ImageView) LayoutInflater.from(getContext())
                            .inflate(android.support.design.R.layout.design_layout_tab_icon, this,
                                    false);
                    addView(iconView, 0);
                    mIconView = iconView;
                }
                if (mTextView == null) {
                    TextView textView = (TextView) LayoutInflater.from(getContext())
                            .inflate(android.support.design.R.layout.design_layout_tab_text, this,
                                    false);
                    addView(textView);
                    mTextView = textView;
    ...
    protected class BaseSlidingTabStrip extends LinearLayout {
        ...
        protected int mIndicatorLeft = -1;
        protected int mIndicatorRight = -1;

        private ValueAnimator mCurrentAnimator;

        BaseSlidingTabStrip(Context context) {
            super(context);
        ...
        void setSelectedIndicatorMargin(int margin) {
            if (mSelectedIndicatorMargin != margin) {
                mSelectedIndicatorMargin = margin;
                ViewCompat.postInvalidateOnAnimation(this);
            }
        }

        void setSelectedIndicatorRadius(int radius) {
            if (mSelectedIndicatorRadius != radius) {
                mSelectedIndicatorRadius = radius;
                ViewCompat.postInvalidateOnAnimation(this);
            }
        }
        ...
        private void setIndicatorPosition(int left, int right) {
            if (left != mIndicatorLeft || right != mIndicatorRight) {
                ...
                mIndicatorLeft = left + mSelectedIndicatorMargin;
                mIndicatorRight = right - mSelectedIndicatorMargin;
                if (mIndicatorLeft < 0) {
                    mIndicatorLeft = 0;
                }
                ViewCompat.postInvalidateOnAnimation(this);
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            if (startLeft != targetLeft || startRight != targetRight) {
                ...
                animator.setInterpolator(createIndicatorAnimInterpolator());
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset,
            ...
            if (tabLayout != null) {
                ...
                tabLayout.setScrollPosition(Tab.INVALID_POSITION, position,
                        positionOffset, updateText);
            }
        }
    ...
    public static class ViewPagerOnTabSelectedListener implements TabLayout.OnTabSelectedListener {
        ...
        public void onTabSelected(TabLayout.Tab tab) {
            mViewPager.setCurrentItem(tab.getPosition(),
                    Math.abs(mViewPager.getCurrentItem() - tab.getPosition()) <= 1);
        }
